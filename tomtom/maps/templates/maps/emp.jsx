...
function EmployeeData() {
    ...
    async function searchEmployeeHomeAddress(query) {
        try {
            const response = await fetch(`/maps/search_employee_home_address/?query=${query}`);
            ...
            if (data.success) {
                ...
                console.error('Error fetching employee home address suggestions:', data.error);
            }
        } catch (error) {
    ...
    return (
        <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
                <label htmlFor="home_address" className="form-label">Home Address<span className='mandatoryStar'>*</span></label>
                <TextField
                    type="text"
                ...
                <ul>
                    {homeAddressSuggestions.map((suggestion, index) => (
                        <li key={index} onClick={() => handleAutocompleteSelection('home', suggestion)}>
                            {suggestion.name}
                        </li>
                    ))}
                </ul>
